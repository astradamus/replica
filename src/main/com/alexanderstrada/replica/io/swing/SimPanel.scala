...
private class SimPanel(world: World, display: Display) extends JPanel() {
  ...
  private def fillBlack(g: Graphics): Unit = {
    g.setColor(Color.BLACK)
    g.fillRect(0, 0, getWidth, getHeight)
  }

  private def drawArableMap(g2d: Graphics2D): Unit = if (display.alphaArableMap > 0.0f) {
  ...
  private def drawWorldBounds(g2d: Graphics2D, stdComp: Composite): Unit = {
  ...
  private def drawPlantsAndFruits(g: Graphics, g2d: Graphics2D): Unit = if (display.alphaPlant > 0.0f) {
  ...
  private def drawPlantOrFruit(g: Graphics, color: Color, pos: Vector2d, size: Double, hasCanopy: Boolean = false): Unit = {
